...
function isEspruino() {
	if (typeof process === 'undefined')
		return false;
	if (typeof process.env.BOARD === 'undefined')
		return false;
	return true;
}
...
function blynkHeader(msg_type, msg_id, msg_len) {
	return String.fromCharCode(msg_type, msg_id >> 8, msg_id & 0xFF,
...
var Blynk = function(auth, options) {
	...
	if (options.board) {
		...
		[ bl_node.BoardMRAA, bl_node.BoardOnOff, BoardDummy ].some(function(b) {
			try {
				self.board = new b();
				return true;
			} catch (e) {
...
Blynk.prototype.onReceive = function(data) {
	...
	while (self.buff_in.length >= 5) {
		...
		var msg_id = self.buff_in.charCodeAt(1) << 8
				| self.buff_in.charCodeAt(2);
		var msg_len = self.buff_in.charCodeAt(3) << 8
				| self.buff_in.charCodeAt(4);
		...
		if (msg_type === MsgType.RSP) {
			// console.log('> ', string_of_enum(MsgType, msg_type), msg_id,
			// string_of_enum(MsgStatus, msg_len));
			if (!self.profile) {
				if (self.timerConn && msg_id === 1) {
					if (msg_len === MsgStatus.OK
							|| msg_len === MsgStatus.ALREADY_REGISTERED) {
						...
						self.sendMsg(MsgType.INTERNAL, [ 'ver', '0.4.7', 'dev',
								'js' ]);
						self.emit('connect');
					} else {
						console.log('Could not login:', string_of_enum(
								MsgStatus, msg_len));
						// if invalid token, no point in trying to reconnect
						if (msg_len === MsgStatus.INVALID_TOKEN) {
							// letting main app know why we failed
							self.emit('error', string_of_enum(MsgStatus,
									msg_len));
							// console.log('Disconnecting because of invalid
							// token');
							self.disconnect();
							if (self.timerConn) {
		...
		/*
		 * if (msg_len) { console.log('> ', string_of_enum(MsgType, msg_type),
		 * msg_id, msg_len, values.join('|')); } else { console.log('> ',
		 * string_of_enum(MsgType, msg_type), msg_id, msg_len); }
		 */

		if (msg_type === MsgType.LOGIN || msg_type === MsgType.PING) {
			self.sendRsp(MsgType.RSP, msg_id, MsgStatus.OK);
		} else if (msg_type === MsgType.GET_TOKEN) {
			self
					.sendRsp(MsgType.GET_TOKEN, msg_id, self.auth.length,
							self.auth);
		} else if (msg_type === MsgType.LOAD_PROF) {
			self.sendRsp(MsgType.LOAD_PROF, msg_id, self.profile.length,
					self.profile);
		} else if (msg_type === MsgType.HW || msg_type === MsgType.BRIDGE) {
			if (values[0] === 'vw') {
...
Blynk.prototype.sendRsp = function(msg_type, msg_id, msg_len, data) {
	...
	if (msg_type == MsgType.RSP) {
		...
		/*
		 * if (msg_len) { console.log('< ', string_of_enum(MsgType, msg_type),
		 * msg_id, msg_len, data.split('\0').join('|')); } else { console.log('< ',
		 * string_of_enum(MsgType, msg_type), msg_id, msg_len); }
		 */
	...
	/*
	 * if (!self.profile) { if (self.timerHb) { clearInterval(self.timerHb);
	 * self.timerHb = setInterval(function(){ //console.log('Heartbeat');
	 * self.sendMsg(MsgType.PING); }, self.heartbeat); } }
	 */
};
...
Blynk.prototype.disconnect = function(reconnect) {
	...
	// starting reconnect procedure if not already in connecting loop and
	// reconnect is true
...
Blynk.prototype.error = function(err) {
	var self = this;
	// if we throw error and user doesn't handle it, app crashes. is it worth
	// it?
